#!/bin/bash
# Deploy openshift-ai-lightspeed using the catalog
...
CATALOG=${1:-openshift-ai-lightspeed-catalog}
CHANNEL=${2:-alpha}

DEST_DIR="${OUTPUT_DIR}/rhoails"
mkdir -p "${DEST_DIR}"
...
for i in $(seq 1 20); do
  CSV_VERSION=$(oc get --ignore-not-found=true packagemanifest openshift-ai-lightspeed-operator -o go-template="{{range .status.channels}}{{if eq .name \"${CHANNEL}\"}}{{.currentCSV}}{{\"\n\"}}{{end}}{{end}}")
...
metadata:
  ...
  name: openshift-ai-lightspeed-operator
  namespace: openshift-lightspeed
spec:
  channel: ${CHANNEL}
