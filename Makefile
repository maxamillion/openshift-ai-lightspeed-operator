...
# quay.io/opendatahub-io/openshift-ai-lightspeed-operator-bundle:$TAG and quay.io/opendatahub-io/openshift-ai-lightspeed-operator-catalog:$TAG.
IMAGE_TAG_BASE ?= quay.io/opendatahub-io/openshift-ai-lightspeed-operator

# The image tag given to the resulting catalog image (e.g. make catalog-build CATALOG_IMG=example.com/operator-catalog:v0.2.0).
CATALOG_IMG ?= $(IMAGE_TAG_BASE)-catalog:$(TAG)

CATALOG_NAME ?= openshift-ai-lightspeed-catalog
...
# Podman is the default for Red Hat / OpenShift environments.
...
# architectures. (i.e. make container-buildx IMG=myregistry/myoperator:0.0.1). To use this option you need to:
# - be able to use podman/docker buildx or podman manifest for multi-arch builds
...
container-buildx: ## Build and push container image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name openshift-ai-lightspeed-operator-builder
	$(CONTAINER_TOOL) buildx use openshift-ai-lightspeed-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Containerfile.cross .
	- $(CONTAINER_TOOL) buildx rm openshift-ai-lightspeed-operator-builder
...
.PHONY: openshift-ai-lightspeed-deploy
openshift-ai-lightspeed-deploy: export OUTPUT_DIR = out
openshift-ai-lightspeed-deploy: ## Deploy using a catalog image.
	bash scripts/gen-catalog.sh $(CATALOG_IMG) $(CATALOG_NAME)
	oc apply -f $(OUTPUT_DIR)/catalog
	bash scripts/gen-rhoails.sh $(CATALOG_NAME) $(CATALOG_CHANNEL)
	oc apply -f $(OUTPUT_DIR)/rhoails
	bash scripts/confirm-rhoails-running.sh

# Undeploy using the catalog image.
.PHONY: openshift-ai-lightspeed-undeploy
openshift-ai-lightspeed-undeploy: export OUTPUT_DIR = out
openshift-ai-lightspeed-undeploy: ## Undeploy using a catalog image.
	find out/{catalog,rhoails} -name "*.yaml" -printf " -f %p" | xargs oc delete --ignore-not-found=true
...
kuttl-test-run: kuttl openshift-ai-lightspeed-deploy kuttl-test openshift-ai-lightspeed-undeploy
